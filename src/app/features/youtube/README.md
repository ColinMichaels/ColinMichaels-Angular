...
- `components/latest-videos` renders a reusable latest-videos section with a channel link, explicit subscription action, and caller-owned heading, section ID, and analytics source. Its default is Colin Michaels; the homepage FPV section, `/topics/drones-fpv`, and eligible drone article endings explicitly request Captain Colin.
...
- The general Colin Michaels feed is configured by `COLIN_MICHAELS_YOUTUBE_CHANNEL_ID` and must remain `UCCJMwxuUIb6S4aoZiZeAVeQ`. The existing Captain Colin feed uses `YOUTUBE_CHANNEL_ID` and must remain `UCKZ3E88t-BoUqPgZygJw6bA`. Functions reject a missing/different parameter or an unexpected API channel, and Angular rejects a callable payload with another channel ID before rendering it.
- Canonical channels, channel links, subscription links, and analytics identities live in `shared/seo/site-identity.ts` for Angular and `functions/src/seo-site.ts` for the isolated Functions build. Changing either identity requires one reviewed change across both contracts, the corresponding Functions parameter, physical homepage graph, content packages, and rollback notes; never switch only the runtime parameter.
- Deploy the callable and HTTP Functions change with the Hosting update. New clients always send an explicit channel key; keyless legacy calls remain Captain Colin for safe rollout and rollback. No data migration is required.
