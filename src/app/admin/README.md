...
Portable-package blocks that already satisfy the canonical blog contract bypass raw Editor.js import parsing, including loose backup wrappers that still carry a legacy string author. This keeps canonical lists and YouTube embeds editable and publicly renderable. When a first save has already created revision 1 but the New Post editor did not adopt the response, the revision-conflict path opens **Recovery & Conflicts** with explicit guidance. **Reload remote** persists the local recovery copy, adopts the latest canonical post, and replaces `/admin/cms/new` with the post's edit route; **Save as new draft** is reserved for an intentional duplicate. Stored compatibility envelopes are recovered only when their list or embed data is unambiguous and passes the normal editor validator.
