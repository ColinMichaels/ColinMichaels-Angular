...
New Post package import accepts a clean portable folder containing exactly one post JSON document, one literal `image-manifest.json` with a top-level `images` array, and only the final image files referenced through exact `media://` placeholders. Source notes, provenance sidecars that use `assets`, and unused working images belong outside the selected import folder. Package discovery ignores slug-only provenance JSON, conflicting post candidates are named in the error, and a successful import uploads media while leaving an unsaved draft for review. A persistent accessible status panel distinguishes package checks, per-image transfer progress, trusted server-side processing, draft preparation, completion, cancellation, and failure; a pre-existing polite live region announces bounded milestones without repeating every percentage or terminal toast, and partial failures report finalized images retained in the Media Library. JSON and package imports share one editor lock, visible
import/export/preview/thumbnail launch controls retain keyboard focus through guarded `aria-disabled`, and save/delete/export/preview plus in-app navigation stay blocked until the import settles. Imports also wait for pending preview/export, canonical or recovery state application, thumbnail work, form-media uploads, and Editor.js uploads so no older completion can replace part of the imported draft. Recovery restore, canonical reload, and Firestore hydration share a serialized editor-state queue with counted ownership; queued hydration rechecks dirty state and revision before applying, while reload rechecks the current remote revision after Recovery persists so a later revision wins and a concurrent deletion aborts safely. Thumbnail generation is mutually exclusive with cover-image upload and rejects repeat generation. Component teardown cancels unfinished package and Editor.js resumable transfers, invalidates the import continuation, and cannot apply a late draft; finalization
already started after upload completion remains server-owned.

Portable-package blocks that already satisfy the canonical blog contract bypass raw Editor.js import parsing, including loose backup wrappers that still carry a legacy string author. This keeps canonical lists and YouTube embeds editable and publicly renderable. When a first save has already created revision 1 but the New Post editor did not adopt the response, the client detects the exact expected-revision-0/found-revision-1 conflict, force-persists the latest local Recovery copy, adopts revision 1, and replaces `/admin/cms/new` with the post's edit route automatically. If Recovery persistence fails, **Recovery & Conflicts** remains open with **Use saved draft** for a safe retry. **Create separate recovery draft** requires confirmation and is reserved for intentionally keeping both versions; revision 2 or another mismatch remains a manual conflict. Stored compatibility envelopes are recovered only when their list or embed data is unambiguous and passes the normal editor validator.
...
The protected Posts screen now includes a read-only **Evidence review queue**. It counts published posts that still expose the legacy unclassified notice, all posts with incomplete evidence metadata, and reviewed posts; its Evidence filter can narrow the table to each state. The queue never assigns or changes evidence in bulk. Editors must open each post individually and use **Evidence & Disclosures** to record the supportable basis, article-specific boundary, source-review date when source-dependent, and any relationship, AI-assistance, or synthetic-media disclosure.

Normal post-editor **Save Post** remains a full-document operation: it collects and validates Editor.js blocks before the trusted post mutation. Existing posts also expose **Save evidence only** inside **Evidence & Disclosures**. That CMS-role-gated operation sends no article blocks, validates and patches only normalized `editorial` metadata plus revision/sync timestamps, records an audit event and idempotency receipt, and rejects stale revisions. It marks only the evidence controls as saved; body, metadata, and Social Shares edits remain unsaved. If an older post reports block JSON, unsupported-block, or document-count errors during a full save, stop rather than deleting or rewriting content and use the evidence-only control only for a genuine disclosure update.

The same screen includes a separate read-only **Discovery & trust review queue**. It projects the existing evidence state plus usable-source, contextual-next-read, and supporting-artifact signals from each post body, counts published and total review rows, and exposes a dedicated filter. **Review published discovery gaps** applies the filter and sorts required evidence/source work ahead of advisory continuation or supporting-material opportunities. Source links are required only when the selected evidence basis depends on sources or the article already names a Sources/References section without a usable URL; contextual links and artifacts remain advisory so first-person journals are not padded with irrelevant citations or filler. The queue does not write Firestore, change status, assign evidence, or apply recommendations in bulk.
...
The page accepts a dated `.json` file, drag-and-drop, or pasted JSON; provides structured editing for prompts, hints, type, difficulty, estimated time, choices, correct answers, explanations, and source evidence; and can load an existing imported multiple-choice set back into the editor. **Preview reader experience** runs any loaded draft locally in the Admin workspace with question navigation, hints, answer feedback, explanations, source links, completion, and restart. Preview activity does not call the public quiz submission boundary and cannot write reader progress, streaks, or points. Automatic title-gap sets remain view-only and require a complete generated JSON replacement. An edited draft can be downloaded without writing Firestore.
...
- `BlogPublishingService` sends canonical post save/delete and Draft Preview issue/revoke operations through the CMS-role-gated `mutateBlogPost` callable. `BlogStorageService` remains the read/serialization adapter and no longer writes `/posts` or `/postPreviews` directly. The Function owns complete-schema validation, expected revisions, slug reservation, idempotent receipts, atomic preview state, and audit records; scheduled publishing reuses the same validator and transaction contract. Unsupported compatibility blocks that contain Firestore-prohibited nested arrays use the lossless `json-v1` envelope during storage and decode back to their original Editor.js data in the editor. Unexpected callable failures are logged without the post body under a scoped reference that the UI can report instead of showing only `INTERNAL`.

The CMS editor extends the base Editor.js toolset with custom `code`, `markdown`, `typography`, `stats`, `chart`, `poll`, `gallery`, `sunoEmbed`, and `html` blocks. The trusted YouTube block also lets an editor mark exactly one embed as the article's companion video; the canonical block keeps that additive marker, duplicate selections are blocked, and the public article prefers the exact deferred video continuation over the generic drone-feed fallback. A selected companion can carry its exact public title, factual description, ISO upload date, and positive runtime in seconds. Only a complete record emits matching nested `VideoObject` data in Angular and Functions SEO output; legacy or incomplete companions still render and never receive guessed metadata. Structured blocks store typed data rather than presentation-only HTML, so public blog rendering and assistant prompts can consume the content consistently. Chart blocks accept flat `chartPoints` rows or standard Chart.js `labels`/
`datasets` JSON, preserve optional series labels during editing, and render each line series independently on public posts. Markdown blocks preserve their source and are parsed through the sanitized public renderer. Editor blocks are styled with hover/focus block-type hints and preview-oriented rendering so drafts better resemble the published post while editing.

The content-safety compatibility layer accepts the original flat `items: string[]` list contract and the recursive Editor.js list contract without requiring a Firestore migration. Recursive list items preserve their content, child items, and checklist state through editor round trips; reading time, search, and assistant text projections include nested content in visual order. An Editor.js block that is not supported by the canonical renderer is retained as an opaque compatibility payload containing its original type, data, and optional tunes instead of being discarded. When a preserved raw block type later becomes supported and its original data passes the current validator, the editor reopens it as that real block; portable-package lists using the canonical `ordered` / `listStyle` / `listItems` contract receive the same lossless recovery. Production Preview then renders the recovered list or chart, and **Save Post** persists the typed public block. No background Firestore migration
occurs. A payload that is still unknown or invalid remains an inert **Compatibility protection** block with **View preserved JSON** and is omitted from public rendering; authors should not remove it unless they intend to remove or separately migrate the original block. Valid unknown blocks raise a preservation warning. Structurally malformed known blocks raise block-indexed validation errors and cannot leave JSON mode or save; direct adapter conversion still envelopes them instead of dropping data.
...
Multi-image content uses the separate `CmsGalleryBlockTool`, with two to twenty images and the bounded `slideshow`, `grid`, or `mosaic` layout presets. Its shared Media Library picker supports ordered multi-selection up to the gallery's remaining capacity, while staged uploads still finalize once per image. The tool requires alt text, preserves optional captions and intrinsic dimensions, and round-trips through a typed `gallery` block. Production Preview and public rendering share `BlogGalleryComponent`; slides never autoplay, layouts retain DOM order, and the shared lightbox limits Previous/Next navigation to the active gallery. See `docs/ARCHITECTURE/EDITORJS_IMAGE_GALLERIES.md` for the persistence, deployment, and rollback contract.
...
- YouTube companion: one trusted YouTube block may be marked as the exact article companion, producing a deferred end-of-article card, optional evidence-backed `VideoObject` data from exact public metadata, and separate privacy-bounded analytics; see `docs/ARCHITECTURE/EDITORJS_YOUTUBE_COMPANION_VIDEOS.md`
...
Manual blog image uploads reuse `BlogMediaUploaderComponent` and `BlogMediaUploadService`, but the trusted storage identity is no longer a client-owned final URL. JPEG, PNG, WebP, AVIF, and GIF inputs are uploaded to the authenticated actor's create-only `cms/blog-media-staging/{uid}/{mediaId}/` path. Because staging reads are intentionally denied, the client treats the successful write callback as completion, skips staging URL resolution, and gives `finalizeBlogMedia` the private Storage path. The Function verifies the stored metadata and actual file signature, enforces the eight-megabyte and forty-megapixel bounds, generates bounded 480/960/1600 AVIF/WebP/JPEG variants without enlargement for cover and article media, and records dimensions/checksums/paths in `/blogMediaAssets`. **Open Graph / Social Share Image** uploads instead create JPEG-only responsive variants and store the largest JPEG URL; the post-package importer requests that same JPEG-only path for manifest entries marked
`open-graph`. For an older post that still stores the finalizer WebP
URL, server-rendered social metadata resolves the immutable JPEG sibling from that media record without changing the published post. Staging and partial variants are cleaned after success or failure, and a repeated finalization request returns the durable record. Existing legacy `cms/blog-media` URLs remain
