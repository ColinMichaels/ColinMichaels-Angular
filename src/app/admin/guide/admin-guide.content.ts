...
export const ADMIN_GUIDE_ENTRIES: readonly AdminGuideEntry[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      {text: 'Use Import post package only with a clean portable folder containing exactly one post JSON document, one image-manifest.json file with a top-level images array, and the final image files referenced by exact media:// values. Keep source notes, provenance manifests that use assets arrays, and unused images outside the selected folder. The import uploads declared images through the Media Library pipeline but leaves the article as an unsaved draft. If package files conflict, the error names the matching documents; if the requested slug already exists, continuing after confirmation creates a separate draft rather than overwriting the existing post.'},
      ...
      {text: 'Add image-specific Alt text and an optional Caption, and retain intrinsic dimensions when the Media Library provides them. In Production Preview, check portrait, landscape, and unusually wide images at Mobile and Desktop widths plus 200 percent Reader text. Open the full-screen image with the keyboard, move through the article’s full-screen image viewer with the arrow keys, close with Escape, and confirm focus returns to the image trigger.'},
      {text: 'Use an Image Gallery block when 2–20 finalized images belong together. Select several Media Library images at once in the order they should appear, upload one or more images, or add safe image URLs; then reorder them, give every image useful Alt text and an optional Caption, and choose Slideshow, Grid, or Mosaic. Gallery blocks store only bounded layout metadata and image references, never file data, arbitrary column counts, autoplay settings, or custom CSS.'},
      {text: 'Review galleries in Production Preview at Mobile and Desktop widths and 200 percent Reader text. Check every image in Grid and Mosaic, operate Slideshow with its visible or focused keyboard controls, open each full image, and confirm captions, reduced-motion behavior, unavailable-image feedback, and focus restoration.'},
      {text: 'For an exact article/video pair, add the trusted YouTube block and select Use as this article\'s companion video. Select exactly one companion block per post. In Companion video search metadata, copy the exact public video title, a factual description, the public upload date, and runtime in seconds; use an ISO date such as 2026-08-13 or a timezone-qualified timestamp such as 2026-08-13T13:43:21Z. Complete metadata adds VideoObject data to both the browser and crawler article HTML. Missing metadata leaves the companion card intact but emits no guessed video schema.'},
      {text: 'Confirm the exact Watch companion video card and playable in-article embed in Production Preview. The site can measure the outbound choice, but it cannot guarantee a Google rich result or add the reciprocal article link to the YouTube description, card, end screen, or pinned comment; those channel changes remain separate manual steps.'},
      ...
      {text: 'In Preview & SEO, review the Discovery & Trust Checklist. Use descriptive external references for verifiable claims, add an in-body link to a genuinely useful ColinMichaels article, and include relevant supporting evidence such as an original photo, screenshot, measurement, comparison table, code sample, or video when available. These content checks are advisory: a first-person journal entry may legitimately have no external citation, and filler should never be added just to turn a warning green.'},
  ...
  {
    category: 'publishing',
    id: 'review-article-evidence',
    keywords: [
      'evidence review queue',
      'discovery trust review queue',
      'published discovery review',
      'discovery priority',
      'usable source links',
      'contextual next read',
      'supporting evidence',
    ...
    links: [{label: 'Open Posts', route: cmsRoute}],
    roles: CMS_ACCESS_ROLES,
    steps: [
      {
        text: 'Open Posts and review the Evidence review queue before starting new editorial work.',
        link: {label: 'Open Posts', route: cmsRoute}
      },
      {text: 'Choose Published needs review first. The Evidence review queue is read-only and never guesses an article’s evidence or disclosures.'},
      {text: 'Open each matching post with Edit. Do not use a bulk status action as a substitute for article-specific evidence review.'},
      {text: 'In Evidence & Disclosures, choose the evidence basis that actually describes the article and write a concise boundary explaining what was tested, observed, researched, or supplied.'},
      {text: 'For Researched analysis, Manufacturer-supplied information, or Mixed evidence, record the date the source-dependent claims were last checked. Add relationship, AI-assistance, or synthetic-media disclosures when they apply.'},
      {text: 'Return to Posts and choose Review published discovery gaps. The read-only Discovery & Trust queue sorts published gaps by required evidence/source work before advisory continuation or supporting-material opportunities.'},
      {text: 'Treat Needs usable source links as required review for source-dependent evidence or a named Sources section without a URL. Contextual next read and supporting evidence remain advisory; skip them when they would be irrelevant or filler.'},
      {text: 'For an existing post, choose Save evidence only when this review changes only Evidence & Disclosures. The protected operation sends no article blocks, advances the expected revision, records an audit event, and leaves title, status, dates, media, taxonomy, Social Shares, body content, and unknown legacy fields untouched.'},
      {text: 'Save evidence only marks only those evidence controls as saved. Any other metadata, Social Shares, or body edits remain unsaved and stay in Recovery. Use Save Post only when you intentionally reviewed and want to save the complete Editor.js document.'},
      {text: 'If the evidence-only action reports a stale revision, stop and review or reload the latest canonical post; do not repeatedly retry or overwrite the newer revision. If a full Save Post reports legacy block JSON or Editor.js validation errors, do not delete or rewrite blocks merely to make a disclosure update pass.'},
      {text: 'After the individual evidence update succeeds, return to Posts and confirm the article moves from Needs review to Reviewed. Then perform any separate Discovery & Trust checklist and Production Preview work required by a complete content change.'},
    ],
    summary: 'Prioritize published evidence, usable-source, contextual-continuation, and supporting-material gaps, then review every article individually.',
    title: 'Review article evidence, discovery, and trust',
  },
  {
    category: 'publishing',
    featured: true,
  ...
  },
  {
    ...
    steps: [
      {
        text: 'Open Homepage and review the current featured-article mode and public media roles.',
      ...
      {text: 'In the post editor, attach a clean landscape Editorial Feature / Post Background without a baked-in headline. It fills the homepage backdrop when that post owns the feature.'},
      {text: 'The normal post image remains sharp in the large article panel. If no dedicated background is attached, the homepage reuses that post image behind the copy with heavy blur and darkening.'},
      {text: 'Keep at least one published Screen Saver / Legacy slide for the optional full-screen viewer and as a placeholder only when neither the dedicated background nor the normal post image is usable. The selected placeholder is stable for each post and does not rotate while the article is shown.'},
      {text: 'Use Legacy client: prefer post background only when preparing an older-client or code rollback; it does not change the current public media roles.'},
      {text: 'Save, then open the public homepage to verify desktop and mobile presentation.'},
    ],
    summary: 'Choose the homepage feature article, its full-bleed backdrop, and the post image shown in the article panel.',
    title: 'Update the homepage feature',
  },
  {
    category: 'site-content',
