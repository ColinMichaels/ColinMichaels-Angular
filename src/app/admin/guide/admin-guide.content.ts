...
export const ADMIN_GUIDE_ENTRIES: readonly AdminGuideEntry[] = [
  ...
  },
  {
    ...
    keywords: [
      ...
      'expected revision 0',
      'adopt revision',
      'automatic first save reconciliation',
      'use saved draft',
    ...
    steps: [
      ...
      {text: 'After choosing the folder, keep New Post open while the package status panel moves through Checking package, Uploading media, Processing media, and Preparing draft. The progress bar combines completed images with the current image transfer percentage, while the status text distinguishes a 100 percent transfer from trusted server-side validation and variant creation. Package import complete means the draft is loaded for review, not saved or published; a stopped import names the failure and reports any finalized images that remain in the Media Library.'},
      ...
      {text: 'Existing flat lists, nested lists, checklist state, ordered-list counters, and supported YouTube embeds are preserved when a post is saved. Review long links and nested lists in Production Preview at Mobile width and 200 percent Reader text. A compatibility-protected list, YouTube embed, or chart reopens automatically when its preserved data now passes the current supported-block contract; confirm it in Production Preview, then use Save Post to persist the repaired public block. If Compatibility protection remains, use View preserved JSON for diagnosis and leave the block intact unless you intend to remove or separately migrate its original content.'},
      ...
      {text: 'New image uploads first enter a private staging path. Trusted media processing verifies the file signature and dimensions, then creates bounded AVIF, WebP, and JPEG variants. Cover and article media use the WebP delivery variant; an Open Graph / Social Share Image upload uses the largest JPEG variant so crawler-facing cards receive a compatible image. A rejected or interrupted upload is not attached to the post; correct the source file and retry rather than pasting a staging URL.'},
      ...
      {text: 'If another editor or backend process changes the post, do not repeatedly retry the stale save. Compare the recovery, reload the latest remote revision when available, or create a separate recovery draft only when you intentionally need two versions. Automatic first save reconciliation handles the exact New Post conflict expected revision 0 and found revision 1: the editor first stores the latest local work in Recovery, adopts revision 1, and moves to the post edit route without creating a duplicate. If Recovery cannot be stored, the panel remains open with Use saved draft so you can retry safely. Create separate recovery draft requires confirmation and leaves both drafts for manual review. Revision 2 or any other mismatch remains a real conflict and is never auto-adopted. If the slug is already reserved, choose a different slug; the service will not overwrite the other post.'},
  ...
  },
  {
    ...
    steps: [
      ...
      {text: 'Archive and Delete are separate retained-record states. Open Archived for archived assets; open the distinct Deleted quick filter and choose Restore to return a deleted record to Ready.'},
      {text: 'Organizer keyboard shortcuts pause while a Media Library dialog, drawer, preview, filter panel, or drag overlay is open and while focus is inside an interactive control. Deleted records expose Restore instead of ordinary metadata and lifecycle actions.'},
  ...
  },
  {
    ...
    keywords: ['comments', 'moderation', 'approve', 'hide', 'restore', 'delete', 'deleted queue', 'trusted commenter', 'saving moderation change', 'duplicate action', 'search author post'],
    links: [{label: 'Open Comments', route: `${adminRoute}/${PATH_NAMES.ADMIN_COMMENTS}`}],
    roles: CMS_ACCESS_ROLES,
    steps: [
      {
        text: 'Open Comments, filter by moderation status, and search within the active queue by author, post, reply context, or comment text.',
      ...
      {text: 'Wait for the inline moderation result before choosing another action. The console permits one moderation request at a time and disables all queue actions while that request is pending.'},
      {text: 'Use the visually separated Delete action to open a confirmation dialog and move a retained record out of public discussion into the Deleted queue. Delete does not physically erase the Firestore record; open Deleted and choose Restore if it should return to approved discussion.'},
  ...
  },
  {
    ...
    keywords: ['users', 'roles', 'permissions', 'claims', 'admin', 'cms admin', 'content editor', 'media manager', 'viewer', 'view as user', 'impersonation', 'troubleshoot', 'diagnose access', 'disable sign in', 'restore sign in', 'delete auth user', 'fake email', 'suspicious signup', 'previous page', 'next page', 'keyboard tabs', 'escape dialog', 'focus restoration'],
    ...
    steps: [
      ...
      {text: 'Use Left/Right Arrow, Home, or End to move between the User management and Points leaderboard tabs. The selected tab receives keyboard focus and becomes the only tab in the ordinary Tab sequence.'},
      ...
      {text: 'Role, access, preview, and points dialogs keep keyboard focus inside while open. Use Escape or Cancel to close without saving; focus returns to the control that opened the dialog.'},
