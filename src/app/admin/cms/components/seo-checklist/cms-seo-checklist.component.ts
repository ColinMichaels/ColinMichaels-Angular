    ...
    >
      ...
      <section class="space-y-3 border border-zinc-800 bg-black/30 p-4">
        ...
        <article class="overflow-hidden border border-zinc-800 bg-zinc-950">
          @if (socialPreviewImage(); as previewImage) {
            <img
              [src]="previewImage"
...
export class CmsSeoChecklistComponent {
  readonly checklistInput = input.required<SeoChecklistInput>();
  protected readonly checklist = computed<SeoChecklistSummary>(() => createSeoChecklist(this.checklistInput()));
  protected readonly searchPreviewTitle = computed(() => createSearchPreviewTitle(this.checklistInput()));
  protected readonly searchPreviewDescription = computed(() => createSearchPreviewDescription(this.checklistInput()));
  protected readonly searchPreviewUrl = computed(() => {
    const checklistInput = this.checklistInput();
    return checklistInput.canonical || checklistInput.generatedCanonicalUrl;
  });
  protected readonly socialPreviewImage = computed(() => createSocialPreviewImage(this.checklistInput()));
  protected readonly checklistSummary = computed(() => {
