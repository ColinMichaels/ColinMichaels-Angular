...
- The renderer-agnostic typewriter queue and its public models now live in `core-os/terminal`. Canonical consumers share one identity-tested root queue while the legacy path remains compatible; prompt, timing, callback, mode, sound, and clear behavior stay covered. CLI rendering/commands, AI chat, sound, OS user, settings, and persistence remain deliberately separate.
- The Core OS Dock now derives application icons from installed/running registry state and uses one-click open/focus/restore with running indicators and a reduced-motion-safe launch bounce. Windows retain runtime-only minimized state, animate toward their matching Dock item, transfer focus correctly, and expose native close/minimize/zoom controls; the existing persisted app-ID list remains unchanged.
- Finder and its filesystem cohort now live in `core-os/filesystem`. The writable version-1 virtual filesystem uses presence-aware seed import and revision compare-and-set, retains the seed tree with stable IDs, persists under the authenticated UID through `AppStorage`, validates stored and candidate data, detects stale-tab revision tokens, serializes mutations, and supports create/rename/move/duplicate, independent multi-window navigation/selection, shared refresh, search, tags, list/grid/column views, Trash/Put Back, confirmed permanent deletion, bounded session undo/redo, and bounded diagnostic/reset recovery. Redo is available through the toolbar and `Shift+Command/Ctrl+Z`, survives a failed replay, and clears after a new saved branch; both history stacks reset on account changes and stale-tab reload. Recovery values with a usable revision remain compare-and-set; revisionless corruption requires the explicit destructive reset warning. Legacy component paths remain identity-compatible, including favorite icon metadata and the deprecated mock-service helpers; no host folder, Firebase upload, or public-site dependency is introduced.
- Finder now resolves files through installed application associations with deterministic exact-MIME, MIME-family, extension, and legacy-kind precedence. Double-click and Enter use the same filesystem-owned dispatch port; Markdown Reader and Music open explicit metadata previews without reading bytes, guessing bundled resources, or changing the current track. Unsupported files and folders retained in Trash receive honest feedback, while actual content access remains separately gated.
...
Continue moving reusable implementation from `src/app/components/game` into `src/app/core-os` in small verified batches. The app-registry/manager, windowing, dock, tray, tooltip, browser-storage, context-menu, terminal-typewriter, and filesystem/Finder packages are complete; the terminal renderer/commands, notification, settings, and shell ownership remain. Finder metadata-preview dispatch and session redo are delivered; the next Finder work is richer drag/drop/keyboard organization, conflict choices, and an actual-content adapter. Optional real-folder access follows only through an explicit Connect Folder gesture, retained-handle permission checks, visible reconnect states, and a strict local-by-default/no-background-upload boundary. Preserve routes, persisted local state, window lifecycle, keyboard behavior, and compatibility exports; do not combine an ownership move with a visual rewrite. Window controller extraction remains a separate internal refactor after the ownership migrations so it cannot silently change drag or resize behavior. See [Core OS Desktop and Finder](../ARCHITECTURE/CORE_OS_DESKTOP.md).
