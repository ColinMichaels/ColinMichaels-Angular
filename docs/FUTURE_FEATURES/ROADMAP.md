...
- Finder and its filesystem cohort now live in `core-os/filesystem`. The writable version-1 virtual filesystem uses presence-aware seed import and revision compare-and-set, retains the seed tree with stable IDs, persists under the authenticated UID through `AppStorage`, validates stored and candidate data, detects stale-tab revision tokens, serializes mutations, and supports create/rename/move/duplicate, independent multi-window navigation/selection, shared refresh, search, tags, list/grid/column views, Trash/Put Back, confirmed permanent deletion, session undo, and bounded diagnostic/reset recovery. Recovery values with a usable revision remain compare-and-set; revisionless corruption requires the explicit destructive reset warning. Legacy component paths remain identity-compatible, including favorite icon metadata and the deprecated mock-service helpers; no host folder, Firebase upload, or public-site dependency is introduced.
