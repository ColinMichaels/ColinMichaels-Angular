...
Updated July 14, 2026. Completed foundation work is recorded here so it is not repeatedly scheduled as future scope.
...
- Do not add or deploy OpenAI or weather API infrastructure. Existing assistant, weather, proxy, secret, and environment references are removal candidates that require a focused dependency and migration review before deletion.
