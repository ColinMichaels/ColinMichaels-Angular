...
7. Launch parameters are updated on an existing application instance and forwarded by `AppWindowComponent` without recreating its embedded component, allowing Dock Trash to focus Finder and navigate to its virtual Trash in one action.
8. Tray logout calls `AuthService.logout()` so Firebase sign-out completes before that service redirects to login; on failure the tray remains open and exposes an alert without claiming the session ended.
