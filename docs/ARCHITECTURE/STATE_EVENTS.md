...
5. Auth changes reset Finder's in-memory tree, navigation, selection, undo, and redo before loading the next account namespace. Finder never guesses ownership of or migrates the former origin-wide key.
6. Finder first-use, recovery, undo, redo, and normal mutations clone or import a candidate, validate metadata/conflicts/structural and UTF-8 byte bounds, serialize behind prior work, perform a presence-aware revision compare, persist through transaction completion, and only then emit a shared refresh signal. Undo transfers the replaced state to the bounded redo stack; redo transfers it back to undo; a new saved mutation clears redo. A shared pending-mutation signal prevents another Finder window from acting on a stale history label, while a dedicated replay lock prevents duplicate history commands. Failed replays keep both rendered state and history intact, while stale-tab reload and account changes clear both stacks. Revision-bearing recovery compares its token; the confirmed revisionless path compares as revision zero, which protects a newer valid nonzero snapshot while allowing the user to replace another revisionless corrupt value. Each Finder window refreshes its own path while retaining independent path-navigation history, selection, search, sort, and dialog targets. Storage failure leaves the prior rendered tree intact; a stale revisioned tab reloads the saved winner and asks the user to retry. IndexedDB provides the normal transaction; the localStorage fallback requires Web Locks or fails closed.
