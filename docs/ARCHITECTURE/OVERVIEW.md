...
- Public media:
  homepage Captain Colin uploads are loaded through a public Firebase callable Function that keeps the YouTube Data API key server-side and returns only display-safe video metadata. The configured parameter, YouTube API response, callable payload, public channel/subscription links, and channel analytics must all resolve to the canonical Captain Colin channel; Functions and Angular fail closed instead of presenting another channel under the established site identity. The public section exposes distinct view-channel and subscribe actions and records bounded video/channel selections through the existing privacy-aware analytics boundary without accessing a visitor's YouTube account.
- SEO rendering:
