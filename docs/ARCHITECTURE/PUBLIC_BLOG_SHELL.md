...
Core OS autofill, menus, app windows, macOS controls, gradients, and motion utilities are descendants of `.core-os-scope`. Public blog media motion is a descendant of `.site-theme-scope`. Window-header, app-window, and dock glass chrome live in the canonical `core-os/windowing` and `core-os/dock` components, allowing deliberate embedded OS chrome without reopening global selectors. Public templates must use the `.btn-*`, `.site-*`, and `.blog-*` primitives instead of `.mac-*` classes.
