...
2. **Application opening and richer organization** — route supported MIME types to registered applications, add drag/drop and keyboard organization, expose copy/move conflict choices and redo, and keep operations transactional from Finder's perspective.
...
- Focused filesystem import, account isolation, candidate/stored validation, conflict detection, persistence-failure, serialized-mutation, organization, Trash/restore, undo, recovery, Finder keyboard/dialog UI, activation-parameter, and compatibility tests.
