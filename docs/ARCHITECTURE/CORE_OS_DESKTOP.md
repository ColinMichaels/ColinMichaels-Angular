...
2. **Actual content opening and richer organization** — add bounded trusted content references to the dispatch contract, then add drag/drop and keyboard organization, expose copy/move conflict choices, and keep operations transactional from Finder's perspective.
...
The session-redo phase adds only bounded in-memory history plus toolbar and keyboard controls. Undo and redo persist ordinary version-1 snapshots through the existing revision compare-and-set contract, but the stacks themselves are never stored. Rolling back the controls and redo stack leaves the most recently persisted snapshot valid and requires no schema, storage-key, Firebase, route, content, or host-filesystem migration.
...
- Focused filesystem import, account isolation, candidate/stored validation, conflict detection, persistence-failure, serialized-mutation, organization, Trash/restore, undo/redo branching and replay failure, recovery, Finder keyboard/dialog UI, activation-parameter, OnPush rerender, lifecycle-to-DOM focus, association-precedence, metadata-preview, dispatch-adapter, and compatibility tests.
