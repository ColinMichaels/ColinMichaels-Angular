...
| Heading, list, and reader-preference styles are not yet one coherent type system | Hierarchy, sticky headings, markers, and large reader scales can conflict | Phases 4 and 5 align type scale, sticky metrics, table of contents, and list presentation |
...
| Client save/publish and scheduled publishing do not yet share a complete trusted validation and revision boundary | Invalid or stale content can be published through a different path | Phase 7 centralizes backend validation, revision checks, publishing, scheduling, and media integrity |
...
Accordingly, the Phase 1 compatibility implementation is validated in isolation, but deployment approval and the program-level production-ready claim remain withheld pending the authorized complete-post export, the known browser defects, the repository baseline failures, and the later phases in this roadmap.
...
- A stale or deleted save retains local state and offers canonical reload when a remote post exists, plus **Save as new draft**. Reload first flushes the private recovery copy; it never silently discards the only recoverable local representation.
...
New CMS image uploads use the actor-owned create-only staging path `/cms/blog-media-staging/{uid}/{mediaId}/{file}`. Storage Rules require a CMS/media role, the caller's exact UID, an allowed declared image type, and an eight-megabyte maximum; clients cannot update or delete staged objects. `finalizeBlogMedia` then verifies the object owner and staging identity, compares declared and stored content types, detects JPEG/PNG/WebP/GIF/AVIF from the actual bytes, reads dimensions with Sharp, and rejects images above forty megapixels.

Accepted images receive bounded 480, 960, and 1600 pixel variants without enlargement. Each available width is encoded as AVIF, WebP, and JPEG with immutable caching, dimensions, byte size, checksum, Storage path, and download identity. The largest WebP becomes the canonical primary URL. A durable `/blogMediaAssets/{mediaId}` record and `/blogMediaAudit` entry are written before staging cleanup; an idempotent replay returns that record even after staging has gone. A failed transformation removes partial generated objects and the staging input instead of leaving an attachable orphan.
