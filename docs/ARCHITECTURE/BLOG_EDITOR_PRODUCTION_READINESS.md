...
Loose backup wrappers may contain a legacy string author while their `blocks` array already uses the canonical blog contract. The importer validates and retains an entirely canonical block array instead of sending it through the raw Editor.js boundary, preventing canonical lists and YouTube embeds from becoming opaque compatibility blocks. For revision-1 drafts created before a New Post save response was adopted, the conflict UI opens automatically; canonical reload persists Recovery, applies the newest remote post, and replaces `/admin/cms/new` with the canonical edit route. The editor adapter can also recover stored canonical list/embed envelopes only when their allowed fields are unambiguous, redundant `items` and `listItems` agree exactly, and the reconstructed Editor.js block passes the existing validator. Unknown, malformed, nested-conflicting, or extra-field envelopes remain opaque and publicly omitted.
