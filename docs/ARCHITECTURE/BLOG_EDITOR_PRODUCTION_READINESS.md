...
Loose backup wrappers may contain a legacy string author while their `blocks` array already uses the canonical blog contract. The importer validates and retains an entirely canonical block array instead of sending it through the raw Editor.js boundary, preventing canonical lists and YouTube embeds from becoming opaque compatibility blocks. When New Post receives the exact expected-revision-0/found-revision-1 conflict, the editor treats it as an acknowledged first save whose response was not adopted: it force-persists Recovery, applies the canonical revision-1 post, and replaces `/admin/cms/new` with the canonical edit route automatically. A Recovery write failure leaves the conflict open with **Use saved draft**; creating a separate recovery draft requires explicit confirmation. Revision 2 or any other mismatch remains a manual conflict so genuine concurrent changes are never auto-adopted. The editor adapter can also recover stored canonical list/embed envelopes only when their allowed
fields are unambiguous, redundant `items` and `listItems` agree exactly, and the reconstructed Editor.js block passes the existing validator. Unknown, malformed, nested-conflicting, or extra-field envelopes remain opaque and publicly omitted.
...
This hardening and its progress panel change only the Hosting client and require no Firestore backfill, Function, Rule, index, Storage migration, or secret. The canonical-block and revision-adoption repair is also Hosting-only and requires no data migration. Rolling back the Hosting artifact removes the progress presentation and restores the earlier broad folder classifier, non-reactive busy flag, and compatibility behavior; already-correct portable packages remain compatible with either build, and finalized assets, already-created post documents, or account-local recovery copies are not changed by the rollback.
...
- Unsupported or unrecognized Editor.js blocks become canonical `type: 'unsupported'` blocks whose `data.unsupportedBlock` retains `originalType`, opaque data, and optional opaque tunes. Firestore-safe objects keep the legacy `originalData` / `originalTunes` fields. When either payload contains an array directly inside another array—such as Editor.js table rows—the writer uses the explicit `json-v1` plus `originalDataJson` / `originalTunesJson` representation. The editor decodes that representation before showing the preserved source, so the exact type, data, and tunes survive without asking Firestore to store an unsupported nested array.
...
Accepted cover and article images receive bounded 480, 960, and 1600 pixel AVIF, WebP, and JPEG variants without enlargement, with immutable caching, dimensions, byte size, checksum, Storage path, and download identity. Their durable primary URL remains the largest WebP, preserving the existing delivery contract. An `open-graph` upload—including an `open-graph` manifest entry in a post package—creates only JPEG variants and stores the largest JPEG URL, so it never relies on scraper-specific WebP support. For older published posts that retain a finalized Firebase Storage WebP in an Open Graph field, the crawler renderer safely extracts the trusted media ID, reads only that immutable media record, and selects its widest HTTPS JPEG sibling without rewriting Firestore content. A missing, malformed, or non-managed record keeps the branded fallback image. A durable `/blogMediaAssets/{mediaId}` record and `/blogMediaAudit` entry are written before staging cleanup; an idempotent replay returns
that record even after staging has gone. A failed
...
The `json-v1` compatibility fix is a coordinated Functions-and-Hosting change: the Function must encode a legacy nested-array envelope before Firestore writes it, and the Hosting client must recognize and decode the response and subsequent snapshot. Deploy the two artifacts as one release before retrying an affected save. A rollback may restore both artifacts together, but an affected post successfully saved with `json-v1` must not be reopened and resaved through an older editor; restore the compatible Hosting build first or use the pre-release post export.
