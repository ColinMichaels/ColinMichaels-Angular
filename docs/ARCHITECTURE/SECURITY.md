...
### Core OS session termination boundary
...
- The Angular app initializes Firebase App Check with a reCAPTCHA Enterprise provider before any Firebase feature service can obtain a Storage client. The public site key comes from the CI-generated environment; local debug tokens remain ignored developer-only configuration. Storage enforcement stays off until a deployed preview and production traffic sample verifies both public media reads and authenticated CMS uploads. App Check adds attestation and does not replace the Storage Rules authorization boundary.
