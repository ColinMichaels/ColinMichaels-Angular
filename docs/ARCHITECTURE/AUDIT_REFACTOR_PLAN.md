...
- `features/blog/services/blog-repository.service.ts`: typed blog facade backed by `BlogStorageService`, whose auth-aware Firestore synchronization and trusted publishing client own remote reads and mutations.
...
- [ ] Move the remaining context menu, notifications, terminal, filesystem, settings, and shell components in separately verified batches.
